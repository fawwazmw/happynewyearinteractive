...
export const Ornaments: React.FC<OrnamentsProps> = ({ mode, count }) => {
  ...
  const { ballsData, giftsData, lightsData } = useMemo(() => {
    ...
    const palette = [gold, silver, purple, cyan, pink];

    for (let i = 0; i < count; i++) {
      const rnd = Math.random();
      let type: OrnamentType = 'ball';
      if (rnd > 0.7) type = 'light'; // More lights/stars
      if (rnd > 0.9) type = 'gift';  // Fewer gifts

      // 1. Target Position: Sphere Surface/Volume
      ...
      const radius = 6.5 + (Math.random() * 1.5); // Slightly outside foliage
      const centerHeight = 5;

      const targetPos = new THREE.Vector3(
        radius * Math.sin(phi) * Math.cos(theta),
        radius * Math.sin(phi) * Math.sin(theta) + centerHeight,
        radius * Math.cos(phi)
      );
